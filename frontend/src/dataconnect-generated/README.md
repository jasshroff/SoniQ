...
## GetSongs
You can execute the `GetSongs` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getSongs(): QueryPromise<GetSongsData, undefined>;
...
```typescript
getSongs(dc: DataConnect): QueryPromise<GetSongsData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getSongsRef:
```typescript
const name = getSongsRef.operationName;
console.log(name);
```

### Variables
The `GetSongs` query has no variables.
### Return Type
Recall that executing the `GetSongs` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetSongsData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetSongs`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getSongs } from '@dataconnect/generated';


// Call the `getSongs()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getSongs();

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getSongs(dataConnect);
...
### Using `GetSongs`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getSongsRef } from '@dataconnect/generated';


// Call the `getSongsRef()` function to get a reference to the query.
const ref = getSongsRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getSongsRef(dataConnect);
...
## GetMyPlaylists
You can execute the `GetMyPlaylists` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getMyPlaylists(): QueryPromise<GetMyPlaylistsData, undefined>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getMyPlaylists(dc: DataConnect): QueryPromise<GetMyPlaylistsData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getMyPlaylistsRef:
```typescript
const name = getMyPlaylistsRef.operationName;
console.log(name);
```

### Variables
The `GetMyPlaylists` query has no variables.
### Return Type
Recall that executing the `GetMyPlaylists` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetMyPlaylistsData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetMyPlaylists`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getMyPlaylists } from '@dataconnect/generated';


// Call the `getMyPlaylists()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getMyPlaylists();

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getMyPlaylists(dataConnect);
...
### Using `GetMyPlaylists`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getMyPlaylistsRef } from '@dataconnect/generated';


// Call the `getMyPlaylistsRef()` function to get a reference to the query.
const ref = getMyPlaylistsRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getMyPlaylistsRef(dataConnect);
...
const { data } = await addSongToPlaylist({ playlistId: ..., songId: ..., position: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await addSongToPlaylist(dataConnect, addSongToPlaylistVars);

console.log(data.playlistItem_insert);
...
// Variables can be defined inline as well.
const ref = addSongToPlaylistRef({ playlistId: ..., songId: ..., position: ..., });
